...
public class DND_BattleSceneSetup : MonoBehaviour
{
    ...
    [Header("玩家角色生成点")]
    [SerializeField]
    public List<CharacterSpawnPoint> playerSpawnPoints = new List<CharacterSpawnPoint>();    [Header("敌人角色生成点")]
    [SerializeField]
    ...
    private List<GameObject> spawnedEnemies = new List<GameObject>();private void Awake()
    {
        InitializeManagers();
    }
    ...
    private void StartBattle()
    {
        ...
        foreach (var player in spawnedPlayers)
        {
            if (player != null)
            {
                ...
                if (stats != null)
                {
                    ...
                    Debug.Log($"[BattleSceneSetup] 添加玩家到战斗: {stats.characterName}");
                    
                    // 注册到UI
                    if (DND_BattleUI.Instance != null)
                    {
                        DND_BattleUI.Instance.RegisterCharacterStatusUI(stats);
                    }
                }
            }
        }
        ...
        foreach (var enemy in spawnedEnemies)
        {
            if (enemy != null)
            {
                ...
                {
                    allCharacters.Add(stats);
                    Debug.Log($"[BattleSceneSetup] 添加敌人到战斗: {stats.characterName}");
        ...
        if (allCharacters.Count > 0)
        {
            ...
            {
                Debug.LogError("[BattleSceneSetup] 无法找到CombatManager来启动战斗!");
            }
        }
        else
        {
    ...
    private void SpawnCharactersFromList(List<CharacterSpawnPoint> spawnPoints, List<GameObject> spawnedList, string tag)
    {
        for (int i = 0; i < spawnPoints.Count; i++)
        {
            ...
            if (!spawnPoint.isActive || spawnPoint.characterPrefab == null || spawnPoint.spawnPoint == null)
            {
                Debug.LogWarning($"[BattleSceneSetup] 跳过无效的{tag}生成点 [{i}]");
                continue;
            }
    ...
    {
        foreach (var character in spawnedPlayers)
        {
            if (character != null)
                DestroyImmediate(character);
        }
        
        foreach (var character in spawnedEnemies)
        {
            if (character != null)
                DestroyImmediate(character);
        }

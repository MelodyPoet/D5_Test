...
HorizontalBattleFormationManager 阵型布局:（默认箭簇阵型，类似足球阵型的后排-中场-前锋3段式阵型）从而实现近战职业和远程职业的战斗规则背景，结合角色模板的攻击方式（玩家角色取决于武器）

怪物角色通过Template接口上的战斗熟练项取值+默认物理还是法术攻击等数值来确定攻击方式，写死攻击方式。怪物无武器概念，无法切换装备，伤害直接套用模板上徒手伤害填入数值
...
怪物模板没有装备武器和攻击方式切换概念，通过模板上熟练项决定攻击方式，用徒手伤害骰来代替其武器伤害骰（简化怪物数值设计）
...
8. 区分近战和远程职业方式: 玩家和队友角色通过持有的武器和模板上属性来区分近战和远程职业，怪物角色无装备概念，只通过模板上属性熟练度参数来区分近战和远程职业
9. 近战武器和长触及武器的攻击行为是走到对方面前近战攻击，攻击完后走回原位

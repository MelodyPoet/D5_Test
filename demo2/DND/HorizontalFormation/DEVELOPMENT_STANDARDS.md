...
角色如果手持近战（通过装备/物品的标签判断）武器，只能攻击最多距离自身2个纵深位置的敌人；例如：站位在前排标签的角色手持长剑（近战），只能攻击到对方的前排标签的位置和中场标签的位置；站在中场标签位置的角色手持长剑（近战），只能攻击到对方的前排标签的位置。
...
远程武器可以跨位攻击所有位置的敌人，但如果目标距离自身超过3个纵深位置(超出射程)或者只有1个纵深位置（抵近射击），则自身攻击被强加劣势，例如：如果角色站在后排持远程武器或法术远程弹道攻击时，可以攻击对方阵型所有位置，但攻击对方后排位置时由于超过距离自身3个纵深位置，因此被强加劣势；另一方面，如果角色站在前排标签位置使用远程武器或法术远程弹道攻击对方前排位置时由于距离过近，则被强加劣势（远程武器适合跨位攻击克制近战武器，但被手持近战武器和长触及武器的单位近身原则），同时如果使用法术或者远程武器时，自身位置邻接对方阵型单位，则会受到对方单位的借机攻击。并且当前手持远程武器攻击时，单位攻击类型是站在原地攻击（同法术攻击）

如果角色手持近战武器，且距离自身一个纵深的对方阵型位置单位执行施法动作，则会被强加劣势，且可以对敌方施法单位执行一次借机攻击。
...
9/. 近战武器和长触及武器的攻击行为是走到对方面前近战攻击，攻击完后走回原位，且近战武器只能攻击邻接自身一个纵深的敌人阵型位置，长触及武器则可以攻击最远到距离自身3个纵深的敌人阵型位置
10/. 远程武器或者法术攻击可以跨位攻击敌方阵型任何位置；攻击行为是原地远程攻击，不需要走到对方面前
11/. 角色预制体下方显示当前受到的状态(如燃烧，中毒等)图标，状态持续时间，以及当前的buff/debuff状态图标，从左到右排列，右键图标会弹出文字提示框描述具体状态效果，持续回合数等信息
12/. 角色预制体下方显示目前受到什么敌人的近战武器威胁图标：提示玩家当前你是否由于手持远程武器或者自身是施法单位从而在近战中处于劣势
...
双持客专长允许副手武器的追加攻击可以叠加熟练加值，另外双武器战斗风格专长可以允许你的副武器伤害增加你的属性调整值；
  ...
  - 反应(reaction)是特定条件触发的即时行动，每轮只能使用一次(可以根据特定专长使用一些针对敌人和队友的特殊攻击或防御机制)；默认触发借机攻击所用的资源。

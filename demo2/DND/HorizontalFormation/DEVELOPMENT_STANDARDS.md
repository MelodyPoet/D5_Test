...
14/. 借机攻击原则上要满足以下2点前置条件：1.己方通过长触及/远程/法术进行跨位攻击对方某个在自身当前武器威胁范围内的位置时候；2.同时对方这个位置的前一个纵深位置的单位的近战武器可以威胁到自身，此时对方可以对自身执行一次借机攻击(让玩家跨位攻击对方中场和后排风险和收益产生博弈机制)，借机攻击命中和正常攻击一样再加上此时获得的优势，伤害计算和状态更新和正常攻击一样
...
E	借机攻击	222/224/228	跨位攻击跨越敌方邻接纵深时触发；当前代码仅有日志，待实现
...
H	换武器动作（仅玩家）	232、324	玩家花一回合切换武器以解除劣势/换攻击方式；怪物不能（无装备），属未来策略层
I	AI 目标选择按攻击类型	266	AutoBattleAI 决策阶段应基于 GetAttackStyle 选目标，而非排位

4. 战斗时间表现:屏幕正上方有一个参考《博德之门3》的时间轴，从左到右排列当前行动单位的头像Icon并持续更新进度条，表示当前是轮到谁行动,一回合表示所有敌我单位按照先攻顺序排序全部执行完毕，然后重新先攻骰再次排序。一轮表示轮到当前先攻顺序的单位执行自身本轮的动作。
...
- 借机攻击机制：在对方执行某些动作且自身满足以下条件时可以使用反应动作每回合执行一次借机攻击，借机攻击的伤害计算和暴击机制与普通攻击相同
  ...
  2.当距离你一个纵深的敌方阵型位置上的对方单位手持长触及武器攻击时，且你自身持有近战武器（包括长触及武器也算近战武器）可以使用反应动作对其执行借机攻击;

...
- **方式 1 - 容器模式**: 使用空 GameObject 容器 + 动态实例化 prefab
  - 优势：组织清晰，便于批量管理
  - 适用：需要运行时动态生成不同数量的角色
- **方式 2 - 直接加载**: HorizontalBattleFormationManager 直接动态实例化到场景根节点
  - 优势：减少层级，简化结构
  - 适用：固定数量的角色，不需要复杂的父子关系管理

**技术实现**:

- **推荐使用方式 1**: 容器模式更符合 Unity 最佳实践
- **HorizontalBattleFormationManager**: 可以通过脚本配置的 prefab 数组在运行时动态加载

...
2. 横版阵型战斗系统(更新)
战场站位/武器威胁范围/跨位攻击/借机攻击原则:

HorizontalBattleFormationManager 阵型布局:（默认箭簇阵型，类似足球阵型的后排-中场-前锋3段式阵型）从而实现近战职业和远程职业的战斗规则背景，结合角色模板的攻击方式（无论玩家/敌人角色均取决于武器攻击方式或是否具备法术攻击能力）

玩法设计立意：前排/中场/后排（Front/Middle/Back）三排标签来表示纵深队形来细化战斗规则和玩法机制
(中场只有中路Center一个位置，前排和后排区分左翼Left/右翼Right机制)

武器威胁范围攻击原则：根据角色站位和目标站位与自身纵深距离以及当前所持武器的威胁范围来决定当前单位能攻击的范围

近战武器在近身距离克制远程武器（对方会受到近战劣势惩罚且攻击时候会吃借机攻击），近战武器只能攻击距离自身一个纵深位置的原则:

角色如果手持近战（通过装备/物品的标签判断）武器，只能攻击最多距离自身2个纵深位置的敌人；例如：站位在前排标签的角色手持长剑（近战），只能攻击到对方的前排标签的位置和中场标签的位置；站在中场标签位置的角色手持长剑（近战），只能攻击到对方的前排标签的位置。站在后排标签位置的角色手持近战，无法执行攻击动作（对方阵型任何位置都超过自身攻击距离）。

跨位攻击：当角色手持长触及武器/远程武器/法术远程弹道攻击时，可以攻击距离自身1个纵深位置以外的敌人。但需承担受到距离自身最近一个纵深的敌人对你进行借机攻击的风险。

角色如果手持长触及（长矛/长枪类）武器（通过装备/物品的标签判断），最远可以攻击距离自身3个纵深位置的敌人。例如:站在前排标签的角色手持长触及武器，可以攻击到对方阵型所有位置；站在中场标签位置的角色手持长矛，可以攻击到对方的前排标签的位置和中场标签的位置。且如果跨位攻击会受到对方邻接自身一个纵深位置（如果这个位置有对方单位的话）的借机攻击，例如:站在前排标签位置且手持长触及武器，攻击对方中场标签位置，由于直接跨过了对方前排标签位置，因此对方前排单位可以对你进行借机攻击。但如果自身站在己方阵型中场，由于不直接邻接对方阵型任何单位（躲在己方前排队友后面攻击对方阵型前排或者中场，则不会受到借机攻击）。不跨位攻击则不吃借机攻击。

远程武器可以跨位攻击所有位置的敌人，但如果目标距离自身超过4个纵深位置(超出射程)或者只有1个纵深位置（抵近射击），则自身攻击被强加劣势，例如：如果角色站在后排持远程武器或法术远程弹道攻击时，可以攻击对方阵型所有位置，但攻击对方后排位置时由于超过距离自身4个纵深位置，因此被强加劣势；另一方面，如果角色站在前排标签位置使用远程武器或法术远程弹道攻击对方前排位置时由于距离过近，则被强加劣势（远程武器适合跨位攻击克制近战武器，但被手持近战武器和长触及武器的单位近身原则），同时如果使用法术或者远程武器时，自身位置邻接对方阵型单位，则会受到对方单位的借机攻击。如果角色站在中场标签位置使用远程武器或法术远程弹道攻击对方所有位置均在自身射程内因此正常攻击没有超射程惩罚，但会受到对方前排/中场持近战/长触及武器攻击时对方获得攻击优势（在对方近战武器威胁范围内）。另外当前手持远程武器攻击时，单位攻击类型是站在原地攻击（同法术攻击）

如果角色手持近战武器，且距离自身一个纵深的对方阵型位置敌方单位执行远程攻击或法术动作，则会获得优势，且可以对敌方单位执行动作时己方执行一次借机攻击。借机攻击是在对方执行轮进行的反制机制，和自身攻击轮的动作行为逻辑是分开2条不同逻辑的。

战场敌我双方的角色攻击时候，在当前规则下（玩家站位+当前武器/怪物站位+模板设置）的可攻击位置单位下方显示红圈标记，表示当前可以攻击到的位置。

如果己方和对方阵型对应位置角色战斗中倒下，或者失去了当前武器可以威胁到的位置，则本轮无法执行攻击动作。应该当前角色本轮执行切换对应威胁范围的武器或其他攻击方式如法术等后才可以继续攻击。

角色攻击方式与职业和阵型站位无关，只和手持何种武器（近战/长触及/远程）或者是否配置了模板上的法术攻击优先有关。（角色站在前排也可以使用远程武器或者法术攻击，但会有攻击惩罚：距离对方前排过近从而强加劣势，并且攻击时会吃邻接的敌人借机攻击）通过这样来指引玩家进行策略博弈。

因此之后应预留给到玩家设置角色的AI（不同情况使用不同攻击/队形更换策略）

队战阵型，从左到右双方互相镜像纵深队形，纵深站位和自身攻击方式决定能攻击对方纵深的
什么位置。

横轴从屏幕左到右表示战场纵深位置层级关系（上述武器攻击方式以下面纵深层级关系为准）

竖轴从屏幕上到下表示玩家和敌人角色位置在阵型中的相对左/中/右关系

玩家阵型                                       敌人阵型

后排：左翼         前排：左翼         ←→        前排: 左翼          后排:左翼
Back:Left         Front:Left                  Front:Left         Back:Left
        
        中场：中路      前排：中路    ←→   前排: 中路     中场:中路
        Middle:Center  Front:Center      Front:Center   Middle:Center

后排：右翼         前排：右翼         ←→        前排: 右翼          后排:右翼  
Back:Right        Front:Right                 Front:Right        Back:Right
...
1/. 决策阶段: AI分析可攻击目标列表
2/. 目标选择: 根据手持武器的攻击类型或者是否具备法术攻击来决定攻击目标
...
8/. 区分近战和远程职业方式: 玩家(含队友)和敌人单位均通过持有的武器和模板上属性来区分近战和远程职业
9/. 近战武器和长触及武器的攻击行为是走到对方面前近战攻击，攻击完后走回原位，且近战武器只能攻击邻接自身一个纵深的敌人阵型位置，长触及武器则可以攻击最远到距离自身3个纵深的敌人阵型位置,但是长触及武器跨位攻击时会受到对方邻接自身一个纵深位置（如果这个位置有对方单位的话）的借机攻击
10/. 远程武器或者法术攻击可以跨位攻击敌方阵型任何位置；攻击行为是原地远程攻击，不需要走到对方面前，但超过距离自身4个纵深位置由于射程过远，自身攻击会强加劣势，且攻击时会吃邻接的敌人（如果有敌人的话）的借机攻击
11/. 角色预制体下方显示当前受到的状态(如燃烧，中毒等)图标，状态持续时间，以及当前的buff/debuff状态图标，从左到右排列，右键图标会弹出文字提示框描述具体状态效果，持续回合数等信息
12/. 角色预制体下方显示目前受到什么敌人的近战武器威胁图标：提示玩家当前你是否由于手持远程武器或者自身是施法单位从而在近战中处于劣势；无论站位在哪，只要手持远程武器且直接邻接对方单位，你的攻击都会被强加劣势且在自身攻击轮受到敌方邻接单位借机攻击（除非有专长或者武器特殊能力修正），需花一回合切换近战武器攻击才解除劣势
13/. 优势/劣势机制：当角色处于优势状态时，攻击检定时掷2d20取高；处于劣势状态时，掷2d20取低；当同时处于优势和劣势时，互相抵消，正常掷1d20
14/. 借机攻击原则上要满足以下2点前置条件：
1.己方攻击时暴露在敌方近战武器威胁范围内，按攻击方式区分：
　- 长触及近战(Reach)：仅当**跨位攻击**（目标处于非相邻纵深，纵深距离>=2）时暴露，触发邻接敌人借机（攻击相邻纵深不暴露）；
　- 远程(Ranged)/法术(Spell)：手持远程武器或施法时**始终暴露**，无论攻击哪个位置的敌人（对应 272：邻接敌方即受借机），不要求跨位；
　- 普通近战(Melee)：攻击相邻目标不暴露，不触发借机。
2.同时对方这个位置前一个纵深位置的单位的近战武器可以威胁到自身（邻接 dist=1 且持近战/长触及），此时对方可以对自身执行一次借机攻击(让玩家跨位攻击/远程前排单位时与敌方近战产生博弈)，借机攻击命中和正常攻击一样再加上此时获得的优势，伤害计算和状态更新和正常攻击一样
15/. 借机攻击可以打断对方的专注类型法术，对方进行一个体质鉴定，该豁免DC=max(10, 半伤),不通过则失去专注，这次法术被本次借机攻击打断，无法施放
16/. 借机攻击执行顺序按照目前的先攻顺序执行，例如：敌人A攻击你的队友B,此时你(该案例中视作C)和队友D在本轮都满足执行借机攻击的前置条件，且都有反应动作资源可以执行借机攻击反制敌人A,且你的先攻排序在你的队友D前，则由你来执行并用掉你的反应动作资源
借机攻击统一为原地播放攻击动画，无需走到对方面前
另外攻击是一个瞬时动作，即使受到借机攻击

对齐战斗机制的必要环节（按规格逐条）

A	模板补 intrinsicAttackType（WeaponType）	209/216（怪物无武器但需分类近战/长触及/远程）	缺失：需给 CharacterTemplate 加字段，作为"无武器时"的攻击分类
B	统一 GetAttackStyle 解析器 + 改 AutoBattleAI 路由	234、266、272	现状是 IsCharacterInFrontRow 按排位判近战/远程（:221），需替换为解析器
C	威胁范围（可攻击位置）计算	214/218/220/222/224/226	待实现：近战=邻接1纵深、长触及=3纵深、远程/法术=全图（需把"纵深层级"落成精确表）
D	劣势/优势规则 + 2d20 掷骰	226/228/278	已实现(AutoBattleAI.ComputeAttackModifiers + 接入 ResolveAttack)：劣势=远程/法术 dist≤1(抵近邻接,272) 或 dist>4(超射程,270/222)；优势=近战/长触及打远程/法术目标且 dist≤武器上限(近战2/长触及3,224/222镜像)；273 取大/取小/抵消已落地；228 无法攻击(威胁范围内无目标)跳过并 UI 日志提示
E	借机攻击	222/224/274/275/276/359	已实现(AutoBattleAI.TryResolveOpportunityAttack + CharacterStats.MakeConcentrationSave + InitiativeEntry.hasUsedReaction)：跨位攻击(长触及/远程/法术,目标纵深距离≥2)触发邻接(dist=1)且持近战/长触及的敌方单位借机攻击；反应器取先攻最高者(276)且消耗反应资源(每轮1次,340)；借机攻击为正常近战攻击含优势(224/274/359)；若命中施放中的专注类法术则 DC=max(10,⌊半伤⌋) 体质豁免,失败打断施法(275)；通用专注系统预留接口(Concentrating/ActiveConcentrationSpell,344/891)
F	B 视觉补"怪物分支"	226（怪物远程/法术也飞弹道）	现状 ResolveAttackPrefab 只认主手武器+法术；怪物无武器→不冒箭。需：无武器时按 intrinsicAttackType + 模板 monsterRangedEffectPrefab 出弹道；法术已能走 defaultCantrip.spellCastEffectPrefab
G	可攻击红圈 + 威胁图标 UI	230、276/277	依赖 C 的威胁范围结果；红圈标可攻击位、图标提示"你被近战威胁→劣势"
H	换武器动作（仅玩家）	232、324	玩家花一回合切换武器以解除劣势/换攻击方式；怪物不能，属未来策略层
I	AI 目标选择按攻击类型	266	AutoBattleAI 决策阶段应基于 GetAttackStyle 选目标，而非排位

4. 战斗时间表现:屏幕正上方有一个参考《博德之门3》的时间轴，从左到右排列当前行动单位的头像Icon并持续更新进度条，表示当前是轮到谁行动,一轮表示所有敌我单位按照先攻顺序排序全部执行完毕，然后进入下一轮。一回合表示轮到当前先攻顺序的单位执行自身本回合的行动。每回合单位有一个动作，一个附赠动作，一个反应动作的资源（2D横版战斗游戏在DND5E基础上扩展自定义）
先攻顺序在本次接敌战斗中只一开始投一次并保持。
...
创建角色时候使用27购点法，且最大不能超过15;
你有27点可以花费在属性值上。每一个数值的费用都在属性值点数花费表中给出。比如，14这个数值会用掉你27点中的7点。
数值  花费  数值  花费 
8     0    12     4 
9     1    13     5 
10    2    14     7 
...
- 动作/附赠动作和反应：
  - 动作(action):是角色的基础行动单位，可以执行下列一系列行为；
    ...
    3. 游荡者可以躲藏(Hide):使用这个动作后，你可以尝试进行隐匿检定，从而让自己潜行起来，躲避敌人的视线并且发动偷袭攻击
    4. 切换当前装备的武器：当有时候你当前所持有的武器和攻击方式无法正常攻击对方单位，且你的背包栏里有携带其他可以攻击的武器（如近战和远程武器切换）,可以花费一回合动作切换武器 
    
  - 附赠动作(BonusActions):有很多职业特性、法术或其他能力允许你在你的回合中执行一种额外的动作，这种动作叫作附赠动作。例如，灵巧动作特性就能够让游荡者执行附赠动作。只有在某个特殊能力、法术或游戏中其他明确了你能以附赠动作来做某事的特性的允许下，你才能执行附赠动作。否则，你没有可以执行的附赠动作。你只能在自己的回合执行一个附赠动作，因此在你拥有多个可执行的附赠动作项时，你必须选择你要执行哪一个。除非该附赠动作项的发生时点有明确规定，否则你可以自行选择在你的回合内何时执行附赠动作。任何能够让你无法执行动作的效应，同样会使你无法执行附赠动作。(如游荡者使用附赠动作进行潜行，或者你副手持有另一把武器或者具有盾牌大师时候，花费附赠动作追加一次副手的攻击，这次追加的攻击不叠加熟练加值，伤害只有武器伤害骰，无属性调整值加成)；
  
双持客专长允许副手武器的追加攻击可以叠加熟练加值，另外双武器战斗风格专长可以允许你的副武器伤害增加你的属性调整值；

  - 反应(reaction)是特定条件触发的即时行动，每轮只能使用一次(可以根据特定专长使用一些针对敌人的特殊反制攻击或针对队友的防御机制)；默认触发借机攻击所用的资源。

  - 休息机制：一次地城关卡探险2-3次短休(根据关卡长度决定)，短休允许角色消耗背包内装箱的食物资源1个，恢复一个生命骰的生命值，长休只在营地阶段的床上休息才可以进行，根据实际生活中的时间流逝按分钟回复生命值和其他职业资源（如法术位、技能点等），为后续营地建造玩法预留

法术专注:
有些法术是需要施法者每轮使用固定专注法术位来维持的，当施放了一个专注类别的法术后再下一轮又施法了另一个专注类别的法术，则前一个法术失效而维持当前法术到专注位。如果施法者在维持专注位的法术时候受到任何来源伤害，则进行专注鉴定，失败则中断该法术的维持（法术被消耗掉）
- 专注法术: 专注法术的豁免DC=max(10, 半伤)
...
- 借机攻击机制：在对方执行某些动作且自身满足以下条件时可以使用反应动作每回合执行一次借机攻击，借机攻击的伤害计算和暴击机制与普通攻击相同
  借机攻击触发条件：
  1.当距离你一个纵深的敌方阵型位置上的对方单位手持远程武器进行攻击时，且你自身持有近战武器（包括长触及武器也算近战武器）可以使用反应动作对其执行借机攻击;
  2.当距离你一个纵深的敌方阵型位置上的对方单位手持长触及武器**跨位攻击（目标处于非相邻纵深）**时，且你自身持有近战武器（包括长触及武器也算近战武器）可以使用反应动作对其执行借机攻击（长触及仅跨位暴露，攻击相邻纵深不触发）;
  3.当距离你一个纵深的敌方阵型位置上的对方单位执行施法动作时，且你自身持有近战武器（包括长触及武器也算近战武器）可以使用反应动作对其执行借机攻击;

- 距离判断: 近战/远程攻击距离限制根据阵型站位和当前攻击方式来决定
...
--unity中通过配置SkinConfig(自定义SO)来进行创建
    定义2个空的列表：第一个是皮肤的槽位skinSlots
        ...
        外层--盔甲武器等实际跟玩法逻辑机制强关联的装备（物品共生链接+物品拼接专长等）          
            ...
            armor(铠甲)--包括胸甲/肩甲/大臂区域，对应区分轻甲/中甲/重甲覆盖更多区域（拓扑结构会不同）
            glove(护腕) --包括小臂和手区域，区分轻甲/中甲/重甲（拓扑结构会不同）
            boots（靴子）--包括小腿和脚区域，区分轻甲/中甲/重甲（拓扑结构会不同）
            belt(腰带)--包括腰封/护档/大腿区域，对应区分轻甲/中甲/重甲覆盖更多区域（拓扑结构会不同）
            cloak(披风)
    ...
    第二个是动画的集合skinAnimations
        动画的名字
        类型
        idle/run/attack/unconscious/knockdown/hit/block这些常规动画
...
角色外观换装/装备/属性自定义-->确认角色职业属性模板/当前外观/初始装备-->开始游戏流程

  1.通过Customization Panel进行外观选择和装备配置
  2.确认玩家职业属性/当前外观/初始装备和各项调整值（包括职业/种族增益），并且序列化
  这些数值。
  3.加载战斗场景，生成主控玩家角色（使用以上确认的各项信息），并在游戏进程中保持更新序列化这些角色的关键信息
  4.战斗场景中，玩家生成时候初始物品栏CharacterInventory和CharacterEquipment，以及CharacterStats，都由以上Customization Panel自定义角色时候玩家选择并确认的各项信息生成，
  这些数据需要在战斗场景中保持更新序列化。
---

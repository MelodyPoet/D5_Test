...
- **绝对禁止创建**：任何 GUIDE、COMPARISON、SUMMARY、FIX、STATUS 等后缀的 md 文件
...
**强关联依赖关系：**
- `IdleGameManager` 必须手动引用 `HorizontalBattleFormationManager` 和 `AutoBattleAI`
- 所有角色预制体必须预配置 `DND_CharacterAdapter` + `SkeletonAnimation` + `CharacterStats`
- 背景滚动通过多个 `ScrollLayer` 组件实现视差效果
...
3. **进度推进** → `stageProgressPercent` 每秒增加10%，模拟探索进度
...
**DND_CharacterAdapter 动画控制器：**
- **统一接口** → 所有角色(玩家/敌人)使用相同的动画适配器
- **动画映射** → 通过 `AnimationMapping` 配置不同角色的动画名称
- **状态管理** → 支持 idle、walk、attack、hit、death、cast 等动画状态
- **自动转换** → 攻击/受击动画完成后自动返回待机状态

**关键动画规则：**
- **探索时** → 只有玩家队伍播放走路动画
- **战斗时** → 双方都必须播放待机动画
- **动作时** → 根据职业类型播放对应的攻击/施法动画序列
...
**多层奖励机制：**
- **波次奖励** → 每完成一波给予30%基础经验和金币
- **阶段奖励** → 完成整个阶段给予100%基础奖励
- **战斗奖励** → 每次战斗胜利给予50%基础奖励
- **等级成长** → 敌人属性随关卡等级动态调整

### 🔧 **技术实现特点**

#### **预制体配置优先原则**
- **强制预配置** → 所有角色组件必须在预制体中预先配置
- **禁止硬编码** → 代码中严禁使用 `AddComponent<>()` 动态添加组件
- **错误处理** → 预制体缺少组件时系统报错而不是自动添加

#### **手动Spawn点管理**
- **完全手动配置** → 所有12个spawn点必须在Scene中手动创建和配置
- **Transform引用** → 通过Inspector手动拖拽Transform引用
- **位置精确控制** → 每个spawn点位置可在Scene视图中精确调整

#### **背景滚动与状态联动**
- **视差滚动** → 多层背景以不同速度滚动营造深度感
- **状态同步** → 探索时自动滚动，战斗时自动停止
- **UV兼容** → 自动检测SpriteRenderer并使用最佳滚动方式

### ⚠️ **当前系统限制**

#### **已知问题**
- **UI模块缺失** → 当前UI相关代码已注释，缺少用户界面
- **动画同步** → 某些情况下动画状态可能出现延迟
- **敌人AI** → 当前敌人AI相对简单，缺少复杂战术

#### **技术债务**
- **旧版Role组件** → 保留了旧的Role.cs但与新系统集成度不高
- **混合架构** → 新旧动画系统并存，需要进一步统一
- **组件依赖** → 某些组件间的依赖关系需要更明确的接口定义

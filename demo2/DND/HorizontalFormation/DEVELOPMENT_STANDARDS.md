...
(中场只有中路Center一个位置，暂时没有分左翼Left/右翼Right机制)
...
远程武器可以跨位攻击所有位置的敌人，但如果目标距离自身超过3个纵深位置(超出射程)或者只有1个纵深位置（抵近射击），则自身攻击被强加劣势，例如：如果角色站在后排持远程武器或法术远程弹道攻击时，可以攻击对方阵型所有位置，但攻击对方后排位置时由于超过距离自身3个纵深位置，因此被强加劣势；另一方面，如果角色站在前排标签位置使用远程武器或法术远程弹道攻击对方前排位置时由于距离过近，则被强加劣势（远程武器适合跨位攻击克制近战武器，但被手持近战武器和长触及武器的单位近身原则），同时如果使用法术或者远程武器时，自身位置邻接对方阵型单位，则会受到对方单位的借机攻击。

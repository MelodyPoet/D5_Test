...
14/. 借机攻击原则上要满足以下2点前置条件：1.己方通过长触及/远程/法术进行跨位攻击对方某个在自身当前武器威胁范围内的位置时候；2.同时对方这个位置的前一个纵深位置的单位的近战武器可以威胁到自身，此时对方可以对自身执行一次借机攻击(让玩家跨位攻击对方中场和后排风险和收益产生博弈机制)，借机攻击命中和正常攻击一样再加上此时获得的优势，伤害只计算武器伤害骰，无属性和其他加值（除非有专长或者武器特殊能力修正）
...
4. 战斗时间表现:屏幕正上方有一个参考《博德之门3》的时间轴，从左到右排列当前行动单位的头像Icon并持续更新进度条，表示当前是轮到谁行动
...
- 借机攻击机制：在对方执行某些动作且自身满足以下条件时可以使用反应动作每回合执行一次借机攻击，借机攻击的伤害计算和暴击机制与普通攻击相同，但不叠加属性调整值
  借机攻击触发条件：

...
HorizontalBattleFormationManager 阵型布局:（默认箭簇阵型，类似足球阵型的后排-中场-前锋3段式阵型）从而实现近战职业和远程职业的战斗规则背景，结合角色模板的攻击方式（玩家角色取决于武器，怪物角色通过模板数值写死攻击方式和站位）

玩法设计立意：前排/中场/后排（Front/Middle/Back）三排标签来表示纵深队形来细化战斗规则和玩法机制
(中场只有中路Center一个位置，暂时没有分左翼Left/右翼Right机制)

跨位攻击原则：根据角色站位和当前所持武器来决定（怪物由模板数值写死）

玩家角色如果手持近战（通过装备/物品的标签判断）武器，则只能攻击距离自身一个纵深位置的敌人；例如：站位在前排标签的中路/左翼/右翼角色手持长剑（近战），只能攻击到对方的前排标签的中路/左翼/右翼位置；站在中场标签位置的角色手持长剑（近战），只能攻击到对方的前排标签的中路/左翼/右翼位置和对方的中场标签位置。

玩家角色如果手持长触及（长矛/长枪类）武器（通过装备/物品的标签判断），则可以攻击距离自身2个纵深位置的敌人。例如:站在前排标签的中路/左翼/右翼位置的角色手持长矛，可以攻击到对方的前排标签的中路/左翼/右翼位置和中场标签的位置；站在中场标签位置的角色手持长矛，可以攻击到对方的前排标签的中路/左翼/右翼位置和中场标签的位置和后排标签的中路/左翼/右翼位置，但持长矛攻击自身一个纵深位置(对方前排的中路/左翼/右翼标签位置)时候由于距离过近，会被强加劣势。

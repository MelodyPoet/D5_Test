...
- 动作和反应：
  动作(action)是角色的基础行动单位，每回合可执行一个动作（近战/远程攻击，防御姿势，施法等）；
  反应(reaction)是特定条件触发的即时行动，每轮只能使用一次(可以根据特定专长使用一些针对敌人和队友的特殊攻击或防御机制)；
- 休息机制：短休息允许角色恢复部分资源，长休息允许角色完全恢复并升级生命值
...
-背包中的物品占格子规则：
  - 参考经典背包类游戏《背包乱斗》和经典传统玩法的俄罗斯方块的拼接方法。
    - 物品有2种大类：可装备在角色身上对应槽位的，以及不可装备只能在背包里的，但是所有物品都统一存在于背包格子系统中。(无专门角色身上的装备栏
    的概念)为后续背包乱斗like玩法结合DND5E战斗系统的职业构筑融合做铺垫。
    - 每个物品应该有最低1x1格子大小。
...
- **通用专长定义**：
  - 当前仅做“通用专长”，不区分职业专属；战士/盗贼/法师/牧师/游侠均可自由选择搭配。
  - 专长以“战术机制触发”为主，不以纯数值堆叠为主。

- **与装备拼接/行列加成的联动**：
  - 装备拼接触发“机制来源”；专长提供触发窗口/触发上限。
  - 行激活：用于提升专长触发频率或轻度技能点微增幅（不改变核心机制）。
  - 列激活：用于微属性加成（HP）。
...
- **通用专长清单（整合版，5e通用专长本地化）：*注意以下所有需求均需满足才能选择该专长，且不计算装备增加的属性值，专长提供的属性提升可以计算在内*
  ...
  - 演员(Actor)：(需要等级4+,魅力13+)
    - 魅力提升1，最多提升到20(Ability Score Increse)。
    - 伪装者(Impersonation)表演相关检定微增幅（如表演、欺骗等），并且可以模仿他人声音和外貌（非战斗机制，更多用于剧情互动）。
    - 拟声者(Mimicry)：可以模仿听过的声音，持续时间短（如1分钟），用于社交场合或特定环境互动。
  - 远动员Athlete：(需要等级4+,敏捷13+)
    - 力量或敏捷提升1，最多提升到20(Ability Score Increse)。
    - 攀爬/跳跃相关检定微增幅（如攀爬、跳跃等），并且在攀爬时速度不受限制。
    - 鲤鱼打挺：处于倒地状态时，当前回合如还有动作可以在本角色行动时使用动作起身，无该专长的角色需在下一回合才能使用动作起身。
  - 冲锋手Charger：(需要等级4+,力量或敏捷13+):
    - 力量提升1，最多提升到20(Ability Score Increse)。
    - 进阶疾走:使用疾走时候，下一回合先攻+2,无此专长的角色使用疾走时下一回合先攻+1,持续1回合。
    - 冲锋攻击:在攻击前如果使用了疾走，则下一回合攻击获得额外伤害加成，伤害值为角色等级的一半（向下取整），每回合只能触发一次。
  - **坚韧意志**：每场战斗第1次豁免检定 +1。
  - **敏捷运动员**：移动相关检定/位移后获得轻微命中或闪避增益（回合首击生效）。
  - **冲锋**：位移攻击后获得一次小额伤害加成（每回合1次）。
  - **机动**：位移攻击后获得短暂闪避微增幅（1回合）。
  - **精准打击**：每回合第1次命中额外 +1 命中（仅一次）。
  - **强力一击**：每回合第1次近战命中追加小额伤害（1次/回合）。
  - **野蛮一击**：伤害骰出现最低值时可重掷一次（每回合1次）。
  - **巨武专精**：重型武器命中时获得小额额外伤害（每回合上限1次）。
  - **长柄武器大师**：持长柄武器时首击获得微增幅（每回合1次）。
  - **双武器精通**：双持时回合首击获得微型命中或小额伤害增幅（每回合1次）。
  - **盾牌大师**：持盾时首个受击获得微减伤或短暂AC提升（1回合）。
  - **重甲专精**：穿重甲时第1次受击减伤 1 点。
  - **中甲专精**：穿中甲时获得轻度AC或豁免微增幅（回合首击前）。
  - **轻甲机动**：轻甲位移攻击后获得短暂闪避微增幅（1回合）。
  - **防御决斗**：使用灵巧武器时回合首个受击获得微型AC提升（1次/回合）。
  - **武器专注**：选择一类武器，回合首击命中微增幅（仅一次）。
  - **远程校准**：连续命中同一目标时命中+1（最多叠2层）。
  - **神射手**：远程攻击在特定条件下获得小额额外伤害（每回合上限1次）。
  - **弩手专家**：使用弩类武器时回合首击获得微增幅（每回合1次）。
  - **潜行者**：未被命中时首击获得轻微命中加成（该回合首击生效）。
  - **法术专注**：法术命中后下一次法术伤害微增幅（不叠属性）。
  - **奥术增幅**：法术普通攻击命中后获得一次小额伤害加值（每回合1次）。
  - **法术狙击**：法术普通攻击在特定条件下获得微型命中增幅（每回合1次）。
  - **专注护持**：专注类效果更不易被打断（以豁免微增幅表现）。
  - **仪式施法者**：非战斗阶段解锁更多仪式用途（战斗内不加数值，仅作为扩展位）。
  - **神射观察**：被动察觉相关检定微增幅（用于潜行对抗）。
  - **鼓舞领袖**：战斗开始时为全队提供一次小额增益（如先攻或命中微增幅）。
  - **医疗学识**：战斗内一次小幅自疗或治疗增益（每场战斗1次）。
  - **酒馆斗殴**：徒手或临时武器命中时获得微型伤害增幅（每回合1次）。
  - **抓握专精**：擒抱相关检定微增幅（若后续实现控制类机制）。
  - **地城探索者**：陷阱/机关相关检定微增幅（战斗外为主）。
  - **语言天才**：社交/交流相关检定微增幅（战斗外为主）。
  - **装备协同**：满足装备拼接条件时，专长触发上限 +1（不叠加数值）。
  - **物品共鸣**：特定形状连接达成时，触发一次微增幅（每回合上限1次）。
  - **坚韧体魄**：被击倒后首次恢复时获得小额临时生命值。
  - **冷静反击**：受击后下一回合首击获得微增幅（每回合1次）。
...
战斗风格专长清单（整合版，5e战斗风格本地化）：
  ...
  - 双武器(Two-Weapon Fighting)：当你使用局游轻型武器词条的武器而得以发动追加攻击时，伤害可以增加你的攻击属性调整值。
  - 巨武器(Great Weapon Fighting)：使用双手近战武器时，伤害骰的任何1和2都视作3
  - 对决(Dueling)：当你使用单手近战武器时，伤害增加+2。
  - 拦截(Interception)：当队友被敌人近战攻击时候，可以使用反应来减少攻击造成的伤害，减少量等于你当前的熟练加值（每回合一次）。
必须手持一块盾牌或者一把简易/军用武器来使用这个专长。
  - 守护(Protection):当队友被敌人远程攻击时候，如果你正手持一面盾牌，你可以使用反应将此次攻击强加劣势给敌人（每回合一次）。
  - 投掷(Thrown Weapon Fighting)：当你使用投掷词条武器攻击命中时，伤害骰+2加值。
  - 徒手(Unarmed Fighting)：当你使用徒手攻击命中时,伤害骰变成1d6+力量调整值。必须没有持有任何武器或盾牌来使用这个专长。

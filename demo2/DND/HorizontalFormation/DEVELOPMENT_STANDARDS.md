...
- **❌ 禁止记录临时内容**：不得在文档中记录逻辑更新过程、debug调试记录等临时性内容
...
**关于角色容器的设计选择**:
- **方式1 - 容器模式**: 使用空GameObject容器 + 动态实例化prefab
  - 优势：组织清晰，便于批量管理
  - 适用：需要运行时动态生成不同数量的角色
  
- **方式2 - 直接加载**: HorizontalBattleFormationManager直接动态实例化到场景根节点
  - 优势：减少层级，简化结构
  - 适用：固定数量的角色，不需要复杂的父子关系管理

**技术实现**:
- **推荐使用方式1**: 容器模式更符合Unity最佳实践
- **HorizontalBattleFormationManager**: 可以通过脚本配置的prefab数组在运行时动态加载
- **性能考虑**: 两种方式性能差异很小，优先考虑可维护性
...
- **背景滚动**: 2D 横版背景从右往左位移，营造前进探索的视觉效果
